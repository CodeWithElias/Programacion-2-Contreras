...
//---------------------------------------------------------------------------
		   ...
		   //EXAMEN DEL PRIMER PARCILAL
void col2(TStringGrid *v, byte f, byte ca, byte cb, byte d){
	if (cb==0) {
	v->Cells[cb][f]=d;
	}  else if (cb>0) {
		col2(v,f,ca,cb-1,d);
...
void Examen(TStringGrid *M, byte f, byte c){
	if (f==0) {
	col2(M,f,0,c,f+1);
	} else if (f>0) {
				Examen(M,f-1,c);
...
void __fastcall TPROGRAMACION_2::EXAMEN1Click(TObject *Sender)
{
Examen(matriz,matriz->ColCount-1,matriz->ColCount-1);
}
//---------------------------------------------------------------------------

			 //EXAMEN PRIMER PARCIAL

byte cantidadimpares(TStringGrid *V, byte c){
byte s;
	if (c==0) {
		if (StrToInt(V->Cells[c][0])%2!=0) {
		s=1;
		} else{
			s=0;
		}
	} else {
		s=cantidadimpares(V,c-1);
		if (StrToInt(V->Cells[c][0])%2!=0) {
		s++;
		}
	}
...
byte sumaimpares(TStringGrid *V, byte c){
byte s;
	if (c==0) {
		if (StrToInt(V->Cells[c][0])%2!=0){
		s=StrToInt(V->Cells[c][0]);
		} else{
			s=0;
		}
	} else {
	s=sumaimpares(V,c-1);
		if (StrToInt(V->Cells[c][0])%2!=0){
		s=(s+StrToInt(V->Cells[c][0]));
		}
	}
	return s;
}

byte examen1(TStringGrid *V, byte c){
byte s;
return s=sumaimpares(V,c)/cantidadimpares(V,c);
}
void __fastcall TPROGRAMACION_2::EXAMEN11Click(TObject *Sender)
{
salida->Caption=examen1(vector,vector->ColCount-1);
}
//---------------------------------------------------------------------------

void col3(TStringGrid *v, byte f, byte ca, byte cb, byte d){
	if (ca==v->ColCount) {
	v->Cells[ca][f]=d;
	}  else if (ca<v->ColCount) {
		col3(v,f,ca+1,cb,d);
		v->Cells[ca][f]=d;
	}
}
void Cargado2(TStringGrid *M, byte f, byte c){
	if (f==0) {
	col3(M,f,c-f,c,f+1);
	} else if (f>0) {
				Cargado2(M,f-1,c);
...
void __fastcall TPROGRAMACION_2::CARGADO2Click(TObject *Sender)
{
Cargado2(matriz,matriz->ColCount-1,matriz->ColCount-1);
}
//---------------------------------------------------------------------------

		 // SUMA DE NUMEROS PARES ITERATIVO

int SumaPares(TStringGrid *v, byte a, byte b){
	int s=0;
	while (a<=b){
		if ((StrToInt(v->Cells[a][0])%2==0)) {
			s=s+StrToInt(v->Cells[a][0]);
		}
		a++;
	}
	  return s;
...
void __fastcall TPROGRAMACION_2::SUMAPARES1Click(TObject *Sender)
{
salida->Caption=IntToStr(SumaPares(vector,0,vector->ColCount-1));
}
//---------------------------------------------------------------------------

	   // SUMA DE NUMEROS PARES RECURCIVO

int SumaPares1(TStringGrid *v , byte a, byte b){
byte s;
if (b==0) {  // caso base
	if (StrToInt(v->Cells[b][0])%2==0) {
		s=StrToInt(v->Cells[b][0]);
	}
}else {   //caso general
	   s=SumaPares1(v,a,b-1);
		if (StrToInt(v->Cells[b][0])%2==0) {
		s=s+StrToInt(v->Cells[b][0]);
		}
}
return s;
}

void __fastcall TPROGRAMACION_2::SUMAPARES2Click(TObject *Sender)
{
 salida->Caption=IntToStr(SumaPares1(vector,0,vector->ColCount-1));
}
//---------------------------------------------------------------------------

			 //NUMERO MAYOR AL FINAL DE UN VECTOR

void Pro3R(TStringGrid *v, byte n){
byte x;
if (n>1){
	Pro3R(v,n-1);
	if (StrToInt(v->Cells[n-2][0])>StrToInt(v->Cells[n-1][0])) {
		x=StrToInt(v->Cells[n-2][0]);
...
void __fastcall TPROGRAMACION_2::MAYORFINAL1Click(TObject *Sender)
{
Pro3R(vector,vector->ColCount);
}
//---------------------------------------------------------------------------

		  //NUMERO MAYOR AL FINAL DE UN VECTOR ALGORITMO ITERATIVO

void Pro3(TStringGrid *v, byte n){
Word i; int x;
i=0;
	while (i<(n-1)){
	if (StrToInt(v->Cells[i][0])>StrToInt(v->Cells[i+1][0])) {
		x=StrToInt(v->Cells[i+1][0]);
		v->Cells[i+1][0]=v->Cells[i][0];
		v->Cells[i][0]=x;
	}
	i++;
	}
...
void __fastcall TPROGRAMACION_2::MAYORFINALITERATIVO1Click(TObject *Sender)
{
Pro3(vector,vector->ColCount);
}
//---------------------------------------------------------------------------
